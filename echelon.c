...
static int fileop_listener(
    ...
    if (action==KAUTH_FILEOP_RENAME) {
        ...
        if (strprefix((const char*) arg0, ECHELON_DSCACHE_PREFIX) || strprefix((const char*) arg1, ECHELON_DSCACHE_PREFIX)) { // ignore activity in blessed dir
#if DEBUG
            printf("echelon: detected rename in blessed dir (ignoring): %s -> %s\n", (const char*) arg0, (const char*) arg1);
#endif
            return KAUTH_RESULT_DEFER; 
        }

#if DEBUG
        printf("echelon: detected rename: %s -> %s\n", (const char*) arg0, (const char*) arg1);
#endif
        ...
        if (strprefix((const char*) arg1, ECHELON_DSCACHE_PREFIX)) { // ignore activity in blessed dir
#if DEBUG
            printf("echelon: detected delete in blessed dir (ignoring): %s\n", (const char*) arg1);
#endif
            return KAUTH_RESULT_DEFER; 
        }
        
#if DEBUG
        printf("echelon: detected delete: %s\n", (const char*) arg1);
#endif
...
extern kern_return_t echelon_start(kmod_info_t* ki, void* d) {
    ...
    if (err == KERN_SUCCESS) {
        ...
        if (err == KERN_SUCCESS) {
#if DEBUG
            printf("echelon: registered ctl with id 0x%x, ref %p\n", gCtlReg.ctl_id, gCtlRef);
#endif
        } else {
